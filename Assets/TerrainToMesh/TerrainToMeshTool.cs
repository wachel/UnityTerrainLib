...
namespace TerrainConverter
{
    ...
    {
        public static Material GetMaterial(Terrain terrain,int layer, Shader shader,LayerProperty property)
        {
        ...
        //对每个格子取MaxN个面积最多的layer，将这些layer的Control合并到一张图上
        ...
        //对每个像素，取影响最大的两个layer，将这两个layer的index写入RG通道，将第1个layer的占比写入B通道
        public static Texture2D BakeTextureIndex(TerrainData terrainData) {
            Material mat = new Material(Shader.Find("Hidden/BakeTextureIndex"));
            RenderTexture rt0 = new RenderTexture(terrainData.alphamapWidth, terrainData.alphamapHeight, 24, RenderTextureFormat.ARGB32);
            RenderTexture rt1 = new RenderTexture(terrainData.alphamapWidth, terrainData.alphamapHeight, 24, RenderTextureFormat.ARGB32);
            ...
            for (int i = 0; i < terrainData.alphamapTextures.Length; i++) {
                ...
                mat.SetTexture("_LastResult", i % 2 == 0 ? rt0 : rt1);
                mat.SetTexture("_SplatAlpha", terrainData.alphamapTextures[i]);
                mat.SetFloat("_StartLayerIndex", i * 4);
                Graphics.Blit(Texture2D.blackTexture, mat);
            }

            RenderTexture rt = RenderTexture.active;
            Texture2D result = new Texture2D(rt.width, rt.height, TextureFormat.ARGB32, false, false);
            result.ReadPixels(new Rect(0, 0, rt.width, rt.height), 0, 0);
            result.filterMode = FilterMode.Point;
            result.Apply(false, true);
            RenderTexture.active = null;
        ...
        public static Mesh CreateMesh(TerrainData terrainData, float[,] heights,List<VecInt3>intVertices,List<VecNormal>vecNormals,List<int[]>indices,int layer) { 
            ...
            if (layer == -1) {//layer==-1表示所有layer以submesh方式创建
                int subMeshCount = 0;
